"""Assemble MP_Badlands' terrain colour map into a PNG so it can be LOOKED at.

Unlike tungsten (two 17424-B tiles, colour FIRST), badlands trails exactly ONE
17424-B tile per primary chunk (probe_badlands_decomp.py), so the colour tile is
the last 17424 bytes. Decoded as 132x132 BC7 via bf6_colormap.decode_tile (DDS
wrap -> Pillow), apron cropped, blitted coarse-first; then each paired chunk's
four child tiles (reversed child order [3,2,1,0]) refine the leaves.

Usage:  probe_badlands_colorrender.py [level] [--size 4096] [--out path]
"""
import os
import struct
import sys
...
def blit(canvas, asum, acnt, arr, x0, z0, x1, z1, lo, hi, size):
    """Nearest-neighbour blit of an RGBA interior array over world box."""
...
def main():
    ...
    out = sys.argv[sys.argv.index("--out") + 1] if "--out" in sys.argv else os.path.join(
        ...
        "Battlefield™ Portal Project", "_cmapprobe", "FIXED_MP_Badlands.png")
    ...
    for n in nodes:
        ...
        buf = C.read(p)
        tile = buf[-TILE:]
        if not any(tile[:256]):          # blank shallow tile: skip, do not paint black
        ...
        used += 1
    # refine with paired child tiles
    for n in nodes:
        if n["g1"] is None:
            continue
        p = M.chunk_path(n["g1"])
