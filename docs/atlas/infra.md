...
**Hide/unhide (2026-08-12):** right click without drag (freelook keeps the press) or H hides the highlighted object — a node goes `visible = false`, a MultiMesh instance is zero-scaled with its original transform kept; restore data lives in NODE METAS (`hp_pick_hidden`, `hp_pick_hidden_insts`) so a plugin reload cannot strand things hidden. Owned (user) nodes are REFUSED — hiding writes `visible`, which serializes. `hide_focus(root)` / `unhide_all(root)` (meta sweep; returns `{nodes, insts, gated}` where gated names invisible ancestor groups) / `_note_gate`. The dock's Unhide button maps gated group names to layer chips via `_layer_chip_name` — the "is its mode on" check. `confirm(root)` / `unconfirm(root)` / `is_confirmed()` switch the stage. `provenance(gs)` renders the focused thing's identity as one line (mesh res, scope, variation, surface+state key, instance, world pos) — what a pinned note carries into the saved log.
