...
- **The decision trace** (beside `C_WRAP_TEXCOORD`): `var _dec_rows/_dec_seen/_dec_mx`, `func decide(mesh, state_key, var_hash, surface, material, rules, extra)`, `func flush_decisions(map_name)`, `func decision_count()`. Instance members, not statics (law C4), mutex-guarded because meshes resolve on the walk's worker threads. Rows dedupe on `(mesh, state key, variation, surface)` so a whole map is a few thousand rows rather than one per instance. Written to `user://mapcontext/<MAP>/decisions.jsonl` from `highpoly_mapcontext.gd` when the props finish. Recorded at the two places rules actually fire: `_wrap_channel_fix` (uv.primary + uv.wrap, every carpaint section AND every section that is not one, so "why is this one normal" is answerable) and `material_for` (alpha.gate, at surface -1 because it resolves per material state). Queried by `hp.py decisions`, joined against game truth by `explain.py`, frozen by `golden.py`.
- `const C_WRAP_TEXCOORD := 0x4F5F0664`, `func _wrap_channel_fix(secs, scope, var_hash, mesh_name)` (near `_alpha_gate`) — post-parse carpaint UV override: resolves each carpaint section's depot record (variation-derived key first, like `material_for`) and swaps `sec.uvs` to the texcoord the flag names (1 ⇒ TC1, 0/absent ⇒ TC3). Called from `_mesh_for_body` right after `read_lod`. See readers.md bf6_meshset for the measurement.
