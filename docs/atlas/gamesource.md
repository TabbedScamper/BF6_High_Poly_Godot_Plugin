...
**L6824-L7257 — `material_for`: the material decision tree.** Owns the ordered dispatch — glass → carpaint → tilepaint → look-key share → smoke → decal → mask/foliage → tint-mask → smoothness → plain `StandardMaterial3D` — plus the variation-derived key rule.
